var Vue = (function (exports) {
  ...
  */
      ...
      return function createApp(rootComponent, rootProps = null) {
          ...
          const app = (context.app = {
              ...
              mount(rootContainer, isHydrate, isSVG) {
                  if (!isMounted) {
                      //创建vnode，其实vnode就是一个对象
                      const vnode = createVNode(
                      //rootComponent第1步中调用createApp(App)传入的对象
                      rootComponent, rootProps //传入一些特殊属性，默认为null
                      );
                      ...
                      if (isHydrate && hydrate) {
                          hydrate(vnode, as, any);
                      }
                      else {
                          // 渲染vnode，渲染到rootContainer中，就是mount传入的第一个参数。
  ...
  function baseCreateRenderer(options, createHydrationFns) {
      ...
      const patch = (n1, // n1 表示旧的vnode，当n1为null就是挂载操作
      n2, // n2 标识新的vnode，根据n2的type进行不同处理
      container, // 将vnode渲染到container上
      anchor = null, parentComponent = null, parentSuspense = null, isSVG = false, slotScopeIds = null, optimized = false) => {
          // patching & not same type, unmount old tree
          // 如果新旧vnode不是同一类型，直接卸载旧的
          ...
          switch (type) {
              ...
              case Static:
                  ...
                  break;
              case Fragment:
                  // 新vnode是Fragment类型，vue3是支持模板内部有多个根的，template内有多个根的时
                  // 在这里进行处理（多个根外部会包裹Fragment）
      ...
      const mountElement = (vnode, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
          ...
          {
              //创建dom元素，type其实就是tag，例如div。内部是通过createElement来创建dom元素的
              // 这个其实可以根据不同的平台，编写不同平台的创建元素的函数，实现跨平台
              // 创建dom元素之后，在el属性上保存一份
              el = vnode.el = hostCreateElement(vnode.type, isSVG, props && props.is, props);
              // mount children first, since some props may rely on child content
              // being already rendered, e.g. `<select value>`
              // 通过vnode内部保存的shapeFlag进行判断，如果是文本类型，就直接是设置操作
              if (shapeFlag & 8 /* TEXT_CHILDREN */) {
                  hostSetElementText(el, vnode.children);
                  // 如果有children属性，那么就调用mountchildren（processFragment执行的那一步）
              }
              else if (shapeFlag & 16 /* ARRAY_CHILDREN */) {
                  mountChildren(vnode.children, el, // 用当前元素作为父元素，将children挂载到当前el上
                  null, parentComponent, parentSuspense, isSVG && type !== 'foreignObject', slotScopeIds, optimized || !!vnode.dynamicChildren);
              }
              if (dirs) {
                  invokeDirectiveHook(vnode, null, parentComponent, 'created');
              }
      ...
      const mountChildren = (children, container, anchor, parentComponent, parentSuspense, isSVG, optimized, slotScopeIds, start = 0) => {
          // 遍历children，对每一个child进行patch操作，因为这里patch的第一个参数是null
          // 所以其实就是进行挂载操作。
      ...
      const processFragment = (n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
          ...
          if (n1 == null) {
              ...
              // from arrays.
              // 由原来的注释可以看出，一个Fragment肯定会有array类型的children属性
              // 这里就是获取Fragment内部包裹的内容，进行挂载
      ...
      n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
          ...
          if (n1 == null) {
              ...
              if (n2.shapeFlag & 512 /* COMPONENT_KEPT_ALIVE */) {
                  ...
                  // 否则就挂载新的vnode到container上
                  mountComponent(n2, container, anchor, parentComponent, parentSuspense, isSVG, optimized);
              }
              //如果n1(旧vnode)有值，就执行更新操作
          }
          else {
              updateComponent(n1, n2, optimized);
          }
      };
      const mountComponent = (initialVNode, //传入的n2（新vnode）
      container, anchor, parentComponent, parentSuspense, isSVG, optimized) => {
          //createComponentInstance根据组件对象创建组件的实例
          // vnode 和 instance有什么区别？？
          //     instance用于保存组件的各种状态：data，setup，computed，methods等
          //     vnode用于创建对应的dom
          ...
          // 调用setupRenderEffect，设置和渲染有副作用的函数
      ...
      const setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, isSVG, optimized) => {
          // create reactive effect for rendering  创建响应式的副作用渲染函数
          // effect来自响应式系统，如果传入的函数中使用了一些可响应式数据，就会收集依赖
          // 在响应式数据改变时会重新执行函数
          instance.update = effect(function componentEffect() {
              ...
              if (!instance.isMounted) {
                  ...
                  // renderComponentRoot，渲染组件模板内部的根，vue3是支持多个根的，所以会在多个根外层包裹一层Fragment
                  ...
                  if (el && hydrateNode) {
                      ...
                      // 递归调用patch，旧vnode是null，所以就是挂载操作
      ...
      return {
          render,
          hydrate,
          createApp: createAppAPI(render, hydrate) //调用createAppAPI函数，会返回创建app的createApp函数
      };
  }
  ...
  function setupComponent(instance, isSSR = false) {
      ...
      // 判断是否是有状态组件（有状态组件，内部有data，methods...）
      const isStateful = isStatefulComponent(instance);
      // 1、初始化定义的props : { message : { type:String } }
      // 也会处理attrs
      initProps(instance, props, isStateful, isSSR);
      // 2、初始化插槽
      initSlots(instance, children);
      const setupResult = isStateful
          ? setupStatefulComponent(instance, isSSR) //3、执行setup，内部就是取出Component的setup属性并且执行。
          : undefined;
  ...
  function finishComponentSetup(instance, isSSR) {
      ...
      if (!instance.render) {
          ...
          if (compile && Component.template && !Component.render) {
              ...
              // 调用compile函数对template进行模板编译，将模板转成AST抽象语法树
              // 根据这个抽象语法树来转化成对应的js的render函数代码，返回一个render函数，保存到Component的render属性上
              // 也就是将template编译成render函数
